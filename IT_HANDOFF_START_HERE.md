...
- Edit Case now includes a structured Full Case Style editor for court heading, division, plaintiff, case-number, and multi-party defendant lines, with alignment, bold, and font-size controls. Values remain backward-compatible line-separated text; formatting metadata is stored separately and applied to DOCX merge output.
