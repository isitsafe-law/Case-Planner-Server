...
A leanness pass removed the Settlement Authority workflow, the orphaned Manager Approvals surface, activity-trail edit capability (append-only now), the Document Templates management screen (generation is unchanged), the admin-adjustable staleness threshold (fixed at 60 days), and `cases.track` as a case-status input. Checklist tasks now carry an `owner_role` so the Legal Assistant dashboard's work queues can exclude Attorney-only tasks; deadlines don't carry this field, and existing template tasks were not auto-classified.
