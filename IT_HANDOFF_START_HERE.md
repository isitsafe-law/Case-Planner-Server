...
8. Run `scripts/validate-portable-manifest.ps1 -ExpectedCommit <intended-short-commit>` against the extracted package to check release identity, then run `scripts/local-package-smoke.ps1`; the smoke test also verifies the manifest matches the running server before testing health and document generation.
...
A leanness pass removed the Settlement Authority workflow, the orphaned Manager Approvals surface, activity-trail edit capability (append-only now), the admin-adjustable staleness threshold (fixed at 60 days), and `cases.track` as a case-status input. The same pass also cut the Document Templates management screen in Settings; that cut was later reverted (upload/versioning/section-overlap-runtime-input config is back) after it turned out to remove the only in-app way to add a template, not just dead weight. Checklist tasks now carry an `owner_role` so the Legal Assistant dashboard's work queues can exclude Attorney-only tasks; deadlines don't carry this field, and existing template tasks were not auto-classified.

The Legal Assistant dashboard (Entra-role-routed, or reachable in the SQLite preview through the Local preview role switcher) composes existing records rather than a second task system — see `docs/legal-assistant-dashboard-audit.md`. Pre-filing tracts can carry a `RowIntakeStatus` tracking ROW/title-attorney stage separately from the internal holder chain; a tract parked with ROW or at a terminal outcome drops out of the dashboards' active set but stays queryable. Attorney reminders are a durable, append-only per-case (optionally per-event) thread, surfaced in the existing Attorney Action Queue rather than a second inbox — no email is ever sent. The Service and Publication section now reads the same `ServiceStatusEngine` bands used elsewhere, plus a publication-proof-outstanding list. Division Overview adds a compact Legal Assistant Coverage panel (waiting-on-attorney, assistant work coverage gaps, event-prep risk, pipeline holder distribution) — counts and exceptions, not an activity ranking; its own service-risk count now reads the same shared engine rather than an independent day-count rule.
...
The current dashboard actionability contract is documented in `docs/dashboard-actionability-policy.md`. The SQLite preview provides Settings → Dashboard Actionability for shared timing defaults and displays clear “Why this is here” explanations on Action Queue, Overdue Work, and Due in the Next 7 Days rows. Attorney-specific overrides remain deferred until Entra identity is available; fixed overdue and access/lifecycle rules remain non-suppressible.
Manager Needs Attention uses the same stored momentum and pipeline-stall defaults, avoiding separate hidden thresholds.

The dashboard redesign audit and KPI definitions are in `docs/unified-dashboard-kpi-audit.md`. The attorney dashboard now uses a responsive card grid with compact KPI tiles, Overdue Work, Upcoming Schedule, Due Soon, secondary Case Insight, and a prominent Action Queue. The large bar panels were removed. Upcoming Schedule shows actual records and routes jury trials/events to Calendar and deadlines to the Work Queue deadline facet. Work-row primary actions remain visible, while due-date and open actions use a labelled More menu. Division Overview keeps compact management measures for Management Attention, hard dates within 90 days, jury trials within 180 days, pipeline stalls, and service risk beginning at day 90. It does not duplicate the attorney panels or present opaque workload rankings.
...
- The legacy primary-attorney projection and multiple legal-assistant rows remain supported in SQLite while the additive attorney-assignment relation is adopted. Supporting attorney assignments are available through the provider/API path; historical free-text service party names remain preserved while the optional Service Log-to-defendant bridge is adopted.
...
- On SQLite startup, a one-time additive migration backfills each existing nonblank `cases.assigned_attorney` value into a `Primary` assignment row. The legacy field is preserved as the compatibility projection, so this does not rewrite or remove existing ownership data.
...
- It also flags attorney assignment names that do not match an active Staff Directory record, including legacy/deactivated names.
...
- Edit Case now exposes a paste-friendly defendants/parties line list for the formatted-header workflow. The fixed court caption, plaintiff, case number, tab stops, and final DEFENDANTS label remain in the attorney-authored DOCX template.
- The existing `Property Description` case field is the full legal description. Document templates can use `{{LegalDescription}}`, `{{PropertyDescription}}`, or `{{Case.LegalDescription}}`; blank values are reported as missing without blocking generation.
- Formatted Word headers can be used as DOCX templates with `{{Case.CourtHeading}}`, `{{Case.CaseNumberLine}}`, and `{{Case.DefendantLines}}`. The uploaded DOCX remains the formatting authority for tab stops, alignment, and spacing.
