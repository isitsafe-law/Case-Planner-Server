...
Work Queue uses the same case-name navigation convention and no longer repeats it as an `Open case` row action. Item-specific actions remain available. Eligible task and deadline dates can be edited inline; generated deadlines retain their existing override/history behavior and service/discovery dates remain controlled by their source records. All Reports tabs now use the shared CSV/Excel export control. Exports include the active report rows and filters, use `CasePlanner_<Report>_<date>` filenames, and are generated from the same permission-filtered data already shown by the report.
The report inventory and current export contract are recorded in [docs/report-export-audit.md](docs/report-export-audit.md). Case List and Upcoming Trials Excel exports now re-run their filtered provider-neutral queries on the server; all export requests identify the report and case scope, and the server validates that scope through the current access service before writing Excel. The remaining aggregate reports can move to server-side query services as their provider-neutral contracts are added.
