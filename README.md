...
On the attorney dashboard, the case name is the case-opening link and the displayed due date is the inline date editor for checklist tasks and deadlines. Ordinary tasks and manual deadlines save directly; generated deadlines use the existing manual-override history, with an optional reason when one is useful. Calculated service and discovery dates remain display-only because their source records control them. The Reports tab's Upcoming Trials view reads active `Jury Trial` hearing events, supports 30/60/90/120/180-day or all-upcoming horizons, and includes primary/additional-attorney and division filters. Legacy case-level trial dates do not create report or dashboard trial rows; the legacy migration preserves them by creating Jury Trial events.

Work Queue uses the same case-name navigation convention and no longer repeats it as an `Open case` row action. Item-specific actions remain available. Eligible task and deadline dates can be edited inline without requiring a reason; generated deadlines retain their existing override/history behavior and service/discovery dates remain controlled by their source records. All Reports tabs now use the shared CSV/Excel export control. Exports include the active report rows and filters, use `CasePlanner_<Report>_<date>` filenames, and are generated from the same permission-filtered data already shown by the report.
The report inventory and current export contract are recorded in [docs/report-export-audit.md](docs/report-export-audit.md). All five report exports now re-run their filtered provider-neutral queries on the server; export requests identify the report and case scope, and the server validates that scope through the current access service before writing Excel.
...
A leanness audit against the goal of fairly comparing this system to Lawtoolbox removed six pieces of dead or low-value functionality: the Settlement Authority workflow (Trial Watch now shows the latest Risk Analysis offer/counteroffer instead), the orphaned/unmounted Manager Approvals surface, edit capability on the activity trail (append-only from here on; history remains readable), the admin-adjustable Needs Attention staleness threshold (now a fixed 60 days with explanatory copy; discovery/trial-prep/trial-watch look-ahead windows stay configurable), and `cases.track` as an input to case-status computation (`CaseStatus` is the directly-set source of truth; the search autocomplete for cases stayed). A seventh cut, the Document Templates management screen in Settings, was restored in full (upload, versioning, section/overlap/runtime-input configuration) after review found removing the only in-app way to add a template was a mistake, not a leanness win — generation and the underlying routes were never removed.
...
The dashboard KPI contract and first implementation audit are recorded in [docs/unified-dashboard-kpi-audit.md](docs/unified-dashboard-kpi-audit.md). The attorney dashboard now uses a responsive card grid: compact KPI tiles, Overdue Work, Upcoming Schedule, Due Soon, secondary Case Insight, and a prominent Action Queue. The former bar panels were removed. Urgency count chips use the shared actionable-work projection and route to matching Work Queue filters. Upcoming Schedule shows actual records with date, type, case, and days remaining; jury trials and other events route to Calendar, while deadlines route to the Work Queue deadline facet. Division Overview keeps a compact management KPI strip without reproducing the attorney bar panels. These are management exception and staffing views, not attorney performance rankings.

The top-level Calendar is the shared case-event view. It defaults to the signed-in attorney when Entra identity is available; SQLite preview mode provides an all-attorney view for testing. It supports 30/60/90/120/180-day and See All ranges, attorney scope, event-type filters, multi-day events, and links back to cases. Events are intentionally not Work Queue items; Work Queue contains tasks, deadlines, discovery, and service work. The manager calendar uses the same event catalog and permission-filtered event feed. Dashboard actionable-work visibility and date boundaries are documented in `docs/dashboard-work-visibility-audit.md`.
...
Case Style is displayed in the lower Case Record section rather than the top Overview area. Because attorney-authored DOCX templates now supply the fixed caption formatting, Edit Case exposes only the paste-friendly defendants/parties line list for this workflow. The existing `Property Description` field is the full legal description and is available to templates as `{{LegalDescription}}`, `{{PropertyDescription}}`, or `{{Case.LegalDescription}}`.
