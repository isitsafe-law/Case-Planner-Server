...
The report inventory and current export contract are recorded in [docs/report-export-audit.md](docs/report-export-audit.md). Export requests identify the report and case scope; the server validates that scope through the current access service before writing Excel. A future server reporting API should re-run permission-aware queries instead of accepting client-provided detail rows when large or sensitive exports are enabled.
