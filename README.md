...
The report inventory and current export contract are recorded in [docs/report-export-audit.md](docs/report-export-audit.md). The generic SQLite-preview Excel writer serializes the already-filtered rows supplied by the report; a future server reporting API should re-run permission-aware queries before generating large or sensitive exports.
