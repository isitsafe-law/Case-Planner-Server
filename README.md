...
The dashboard KPI contract and first implementation audit are recorded in [docs/unified-dashboard-kpi-audit.md](docs/unified-dashboard-kpi-audit.md). The attorney dashboard now leads with five action-oriented measures (Overdue, Due Today, Next 7 Days, Action Queue, and Hard Dates within 90 days), followed by accessible CSS bar summaries for work urgency and upcoming hard dates. Bar values use the shared actionable-work projection and route to matching Work Queue or Calendar views. Division Overview has a separate management KPI strip for Management Attention, hard dates, jury trials, pipeline stalls, and 90+ day service risk, followed by compact hard-date, jury-trial-by-attorney, and pipeline-aging summaries. These are management exception and staffing views, not attorney performance rankings.
