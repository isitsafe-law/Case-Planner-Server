    ...
    <div className="trial-watch-list">
      {ordered.map((t) => {
        ...
        return <article key={t.caseId} className="trial-watch-card" onClick={() => onOpenCase(t.caseId)}>
          ...
          <div className="trial-watch-primary"><div><span>Trial date</span><strong>{formatDate(t.trialDate)}</strong></div><div><span>Days until trial</span><strong>{days ?? '—'}</strong></div><div><span>Current status</span><strong>{t.discoveryStatus || 'Trial preparation'}</strong></div></div>
