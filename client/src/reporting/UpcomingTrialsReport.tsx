...
export function UpcomingTrialsReport({
  ...
  return <section className="ui-table-panel">
    <div className="panel-hd"><h3>Upcoming Trials</h3><div className="ui-title-actions"><span className="pill pill-neutral">{rows.length} jury trial{rows.length === 1 ? '' : 's'}</span><ReportExportActions reportId="upcoming-trials" serverQuery scopeCaseIds={rows.map(({ caseRecord }) => caseRecord.id)} title="Upcoming Trials" filters={{ horizon: horizon === 'all' ? 'all upcoming' : `next ${horizon} days`, attorney: attorney || 'all', division: division || 'all' }} columns={[{ key: 'trialDate', label: 'Trial date' }, { key: 'case', label: 'Case' }, { key: 'jobTract', label: 'Job / tract' }, { key: 'county', label: 'County' }, { key: 'primaryAttorney', label: 'Primary attorney' }, { key: 'additionalAttorneys', label: 'Additional attorneys' }, { key: 'days', label: 'Days' }]} rows={exportRows} /></div></div>
