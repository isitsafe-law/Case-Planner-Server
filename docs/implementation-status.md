...
- Legal Assistant dashboard Phase 1 foundation: Entra profiles now expose a configurable Legal Assistant role (`Authentication:Entra:LegalAssistantAppRole`, default `CasePlanner.LegalAssistant`), actor audit labels resolve to Legal Assistant, and eligible non-manager users route to a distinct operational dashboard using existing case, pipeline, event, work-item, and service data. Current-state findings and the phased event-preparation design are documented in `docs/legal-assistant-dashboard-audit.md`; event-linked preparation work and assistant reminder records remain later phases.
- Legal Assistant event-preparation foundation: existing checklist tasks and deadlines now have nullable provider-parity `RelatedEventId` links, task/deadline editors can associate work with an existing proceeding, and assistant proceeding rows derive linked open/overdue/waiting counts. Existing unlinked work is preserved; template-driven date recalculation, preparation-page navigation, and reminder history remain subsequent slices.
- Legal Assistant event-preparation workspace: assistant proceeding rows open a dedicated preparation page with derived Open/Overdue/Waiting-on-Attorney/Completed counts, active work, collapsed completed work, case navigation, and linked task/deadline creation. It composes existing work records; it does not create a preparation-specific item type.
- Legal Assistant preparation-template support: preparation candidates now use the selected event's date for relative due dates, duplicate suppression is scoped to the selected event, and the preparation page can apply reviewed candidates through the existing task/deadline generation service. The next slice is previewed date recalculation when an event moves, with manual overrides and completed items preserved.
- Legal Assistant event-date recalculation: the preparation page can preview and apply linked-date moves, shifting only open generated work by its existing event-relative offset. Manual overrides and completed items remain unchanged; recalculation is recorded in activity history, and ordinary edits to generated task dates are now recognized as manual overrides.
- Legal Assistant attorney follow-up foundation (superseded below): the preparation page reused the existing case-level Waiting/Action Queue workflow to record an attorney request, requested action, follow-up date, and activity history. It did not imply an email was sent, but it also had no real history - a repeat reminder silently overwrote the single waiting_* record.
- Legal Assistant event-change approval foundation: event date proposals are stored separately from confirmed dates, the preparation workspace can submit a proposed date range, and attorney approval/rejection endpoints preserve the confirmed value until approval. SQL Server migration `070_event_change_requests.sql` adds the provider-side request history table.
- Event-change review UI: the preparation workspace can retrieve the pending proposal, approve or reject it, and automatically recalculate linked preparation dates after approval. Rejection leaves the confirmed event and preparation dates unchanged.
- Pending event proposals now appear on the Legal Assistant upcoming-proceedings rows as a visible `Date proposal pending` indicator, while the confirmed date remains the displayed source-of-truth date until approval.
- Pending event proposals also appear as a compact approval strip in the attorney Action Queue and as a manager attention panel; both use the existing event-change request records and navigate to the case/event context without creating a second approval queue.
- Legal Assistant case scope now uses the existing Staff Directory attorney mappings when the signed-in assistant is linked by account or display name. Primary and supporting attorney assignments are included, and the dashboard provides an all-supported-attorneys or individual-attorney filter.
- Assistant work ownership now supports a named owner for both checklist tasks and deadlines. The dashboard provides compact reassignment/takeover controls, owner choices include active assistants and case attorneys, and changes are recorded as WorkOwnerChanged activity entries.
- SQLite preview mode now exposes a Local preview role switcher in the app bar. Selecting Legal Assistant dashboard exercises the role-routed dashboard without Entra; authenticated Entra profiles retain authoritative role routing.
- Leanness audit, Phase 1 (7 cuts against the goal of fairly comparing this system to Lawtoolbox): removed the Settlement Authority workflow entirely (table, endpoints, tests) - Trial Watch's settlement field is replaced by the latest Risk Analysis offer/counteroffer amount, the real live source of negotiation posture. Deleted the orphaned, unmounted `FilingStatusSection` component while keeping the prefiling-milestone aging data it read (also feeds live Manager Dashboard tiles). Activity trail is now append-only (edit endpoint and UI removed; historical rows remain readable). Removed the Document Templates management screen from Settings while keeping template generation and its server routes. Consolidated the Needs Attention tab's momentum/pipeline-stall thresholds to a fixed 60-day constant with explanatory copy (previously admin-adjustable); discovery/trial-prep/trial-watch look-ahead windows remain configurable. `cases.track` no longer drives case-status computation (`CaseStatus` is the directly-set source of truth), and the three confirmed-dead track vestige columns on checklist/deadline/discovery templates were dropped. Server 456/456, client 246/246 across 34 files.
- Phase 0 (pre-Entra identity and audit fixes): `CaseAccessService`/`CaseAccessEvaluator` now give Legal Assistants the same unrestricted read/write access as a Manager rather than scoping them to supported attorneys' cases at the access-control layer - the narrower default dashboard view stays a client-side (Staff Directory-linked scope) concern. `DeadlineReminderBackgroundService` gained the Triage/Pipeline case-status filter every other deadline-suppression path already had (this service is currently dormant - SQL Server pilot + notifications only - but would have reproduced the "reminders fired for imported cases" bug the moment it's enabled). Chief Counsel sign-off attribution: added `OnBehalfOfDisplay`/`OnBehalfOfRole` to `case_prefiling_milestones` (SQLite + SQL Server pilot + migration 072) so a write can record who acted in the system separately from whose approval it represents, with a "Marking on someone else's behalf?" toggle in the case workspace. Server 459/459, client 248/248 across 34 files.
- Phase 2 (task ownership so one checklist serves both dashboards): added `checklist_items.owner_role` (`Attorney` | `LegalAssistant` | `Either`, default `Either`) to SQLite and the SQL Server pilot store (migration 073), plus a "Shows on" selector in the manual task form. The Legal Assistant dashboard's work queues now exclude Attorney-only tasks (previously showed every open task/deadline for supported cases regardless of type); the Attorney Dashboard/general Work Queue stay unfiltered for now. Deadlines don't carry this field. Every existing task defaults to `Either`, so nothing disappeared from either dashboard on migration; the ~200 existing seeded checklist-template tasks were deliberately not auto-classified from their text (a real domain-judgment call, not a pattern-matching candidate), and `checklist_template_items`/Template Editor classification was deliberately scoped out for a later pass. Server 464/464, client 250/250 across 35 files.
- ROW intake tracking: a new `cases.row_intake_status` field tracks a pre-filing tract's stage relative to ROW/the title attorney - an earlier, orthogonal axis from the internal Legal Assistant → Attorney → Deputy Chief Counsel → Chief Counsel review chain. Title-review rounds are recorded as a new event type on `case_prefiling_review_events` (outcome plus a reviewer prompted fresh each round), and cases parked with ROW or at a terminal outcome (Acquired by Agreement, Project Revised, Withdrawn) drop out of the Attorney/Legal Assistant dashboards' active set while staying fully queryable in the general case list. A compact case-workspace panel shows the current status and round history.
- Legal Assistant Dashboard audit Phase 4 (reminders, docs/legal-assistant-dashboard-audit.md): the case-level Waiting/Action Queue stopgap above is replaced by a durable, append-only `case_reminder_requests` record - one thread per (case, optional related event), each reminder appending history rather than opening a duplicate. Repeated reminders on an open thread update that same thread; resolving it starts fresh on the next ask. Open threads surface as their own Attorney Action Queue entries (reusing the existing queue rather than a second inbox) with a Resolve control, and the Event Preparation page's Remind Attorney control now records against this store instead of the shared case-level waiting fields. No email is ever sent by this feature.
- Legal Assistant Dashboard audit Phases 5-6 (service/publication and manager additions): the assistant dashboard's Service and Publication section now reads real ServiceStatusEngine output (via a new bulk `/api/work-queues/publication-entries` endpoint alongside the existing service queue) instead of an ad hoc `caseStatus`/`servicePerfected` boolean, sorted by severity with the engine's own warning text, plus a publication-proof-outstanding exception list. Division Overview gained a compact "Legal Assistant Coverage" panel (waiting-on-attorney reminder threads via a new bulk `/api/reminders/open` endpoint, assistant-owned open/overdue work, unassigned-work coverage gaps, event-preparation risk, and a pre-filing holder distribution strip) - counts and exception lists only, not a per-assistant activity ranking. The dashboard's own former ad hoc service-risk rule (raw filingDate age, ignoring manual deadline overrides) was replaced with the same shared ServiceStatusEngine bands. Legal Assistants remain unrestricted for write/delete operations (an earlier explicit decision, not revisited by this pass).
- Leanness audit, second pass (follow-up gap audit against the same Lawtoolbox-comparison goal): deleted the orphaned, unmounted `IncomingPipelinePanel.tsx` and its dangling comment references elsewhere; removed the dead `PipelinePromotionGate.RequiresFilingApproval`/`EnsureFilingReady` methods (the still-live `GatedChain` constant they sat next to stayed, since three later call sites in the same file depend on it); removed two SQLite-only `CasePlannerRepository` methods (`ExportCaseNotesAsync`, `GetReferenceLibraryAsync`) superseded by their provider-neutral replacements; and removed the orphaned `/api/case-statuses` endpoint. Also flagged, but deliberately left unfixed pending a separate decision, a dead branch in `SaveCaseInternalAsync`: `overrideApplied` is hardcoded `false` and never set `true`, so the `"FilingGateOverridden"` activity-log entry can never fire even though `CaseRecord.FilingGateOverrideReason` is still captured and sent. Server 452/452, client 246/246.
- Document Templates management screen restored: the same second pass also removed the `/api/document-platform/templates` upload/configuration/version-activate/delete endpoints, reasoning "no current client caller" - which contradicted Phase 1's explicit intent to keep those routes reachable after removing the in-app screen, and left no way at all to add a new template. Caught from user feedback; the four endpoints were restored immediately, and the full original Settings → Document Templates screen (upload, versioning, per-template Advanced disclosure for section/overlap/runtime-input configuration, and the Issue Tags "Used By" cross-navigation into it) was rebuilt in `client/src/App.tsx` against the current file - not a blind revert, since `owner_role`, ROW intake, reminders, and Legal Assistant Coverage had all landed in `App.tsx` since Phase 1. Verified live: template catalog load, Advanced configuration disclosure with real section/version data, and the Issue Tags → template cross-navigation all confirmed working against the running server.
