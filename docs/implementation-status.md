...
- Reports phase includes a top-level read-only report builder with consolidated open-case filters, selectable core columns, sorting, optional grouping, preview, Open Case links, CSV/Excel export, and saved report definitions. Saved definitions persist portable SQLite filters, columns, sort order, and grouping; relative-date filters, seeded reports, and user-scoped sharing remain follow-up work.
