...
- Reports phase includes a top-level read-only report builder with consolidated open-case filters, selectable core columns, sorting, preview, Open Case links, CSV/Excel export, and saved report definitions. Saved definitions persist portable SQLite filters, columns, and sort order; relative-date filters, grouping, seeded reports, and user-scoped sharing remain follow-up work.
