...
public sealed class FileReferenceLibraryStore(PathService paths):IReferenceLibraryStore
{
    ...
    {
        paths.EnsureFolders();var metadata=await ReadMetadataAsync(token);var keys=new HashSet<string>(metadata.Keys,StringComparer.OrdinalIgnoreCase);
        ...
        foreach(var key in keys.OrderBy(x=>x,StringComparer.OrdinalIgnoreCase))
        {
            ...
            var info=metadata.TryGetValue(key,out var custom)?custom:new Metadata{Title=key,FileName=$"{key}.txt"};
    ...
    {
        ValidateKey(key);paths.EnsureFolders();var metadata=await ReadMetadataAsync(token);var fileName=metadata.TryGetValue(key,out var existing)?SafeFileName(existing.FileName,key):$"{key}.txt";
        ...
        {
            Title=existingMetadata?.Title ?? key,
