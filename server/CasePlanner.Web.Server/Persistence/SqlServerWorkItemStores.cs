    ...
    : SqlServerWorkItemStoreBase(connections, accessor), IChecklistStore
{
    ...
    public async Task<List<ChecklistItemRecord>> GetAsync(long? caseId, CancellationToken token = default)
    {
        ...
        await using var command=connection.CreateCommand(); command.CommandText="""
            SELECT id,case_id,phase,task,due_date,status,notes,source_type,is_manual,completed_at,source_kind,source_template_id,source_template_version,source_stage,generated_at,generated_by,row_version
    ...
    public async Task<ChecklistItemRecord> SaveAsync(ChecklistItemRecord model, CancellationToken token = default)
    {
        ...
        if(isNew) command.CommandText="""
            INSERT INTO dbo.checklist_items (case_id,phase,task,due_date,status,notes,source_type,is_manual,created_at,updated_at,completed_at,source_kind,source_template_id,source_template_version,source_stage,generated_at,generated_by)
            OUTPUT INSERTED.id,INSERTED.row_version VALUES (@caseId,@phase,@task,@due,@status,@notes,@sourceType,@manual,@now,@now,@completed,@sourceKind,@templateId,@templateVersion,@sourceStage,@generatedAt,@generatedBy)
            """;
        else{command.CommandText="""
            UPDATE dbo.checklist_items SET phase=@phase,task=@task,due_date=@due,status=@status,notes=@notes,updated_at=@now,completed_at=@completed
